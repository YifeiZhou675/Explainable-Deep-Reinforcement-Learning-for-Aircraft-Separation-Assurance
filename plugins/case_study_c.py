...
def init_plugin():
    ...
    regular = [0.15, 0.15, 0.035]  # regular noise level of longitude (R0,R1,R2)
    tas_lvl = 0.1  # noise level of tas

    positions = np.load('./routes/case_study_c_route.npy')
    ...
    agent.predictor.load_weights('best_model_C_5.h5')
    agent.predictor_saliency.load_weights('best_model_C_5.h5')
    counter = 0
    start = time.time()
...
def update():
    ...
    if not len(traf.id) == 0:
        ...
        #     np.save('route0_C.npy', route0)
        #     np.save('route1_C.npy', route1)
        #     np.save('route2_C.npy', route2)
        #     print('com!!!')

        norm_state,norm_context, intruder_idx = getClosestAC(state,traf,route_keeper,previous_action,n_states,store_terminal,agent,last_observation,observation)
...
def reset():
    ...
    np.save('success_train_C.npy',t_success)
    np.save('collisions_train_C.npy',t_coll)
    ...
    if agent.episode_count > 150:
        ...
        if float(df.rolling(150,150).mean().max()) >= best_reward:
            agent.save(True,case_study='C')
            best_reward = float(df.rolling(150,150).mean().max())


    agent.save(case_study='C')
...
def LEGp_v1(state, num_sample, intruder_idx, traf, agent, onorm_state, onorm_context):
    ...
    for ac_idx in range(state.shape[0]):
        # ac_idx = 2
        agent.draw_aircraft_position_C(state, ac_idx, intruder_idx, ori_policy, max_ac)
        ...
        lon_lvl = np.array([regular[r]] * (num_intruder + 1))  # noise level of longitude
        ...
        for i in range(agent.num_intruders):
            if onorm_context[ac_idx][i][6] != 0.0:
                ...
                if lon_diff < regular[route]:
                    lon_lvl[idx + 1] = lon_diff
                else:
                    lon_lvl[idx + 1] = regular[route]
        var_lon = lon_lvl ** 2 / 3
        var_tas = tas_lvl ** 2 / 3
